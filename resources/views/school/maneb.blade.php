...
<div class="grid grid-main">
    <form method="POST" action="{{ route('school.maneb.numbers') }}">@csrf
        <div class="panel">
            <div class="panel-head"><h2>Candidates</h2>@if($leader)<div class="inline-form"><label class="small strong" for="centre">Centre number</label><input id="centre" type="text" name="centre_number" value="{{ $school->maneb_centre_number }}" style="min-width:110px;width:110px"></div>@endif</div>
            <div class="table-wrap"><table class="table">
                ...
                @forelse ($rows as $r)
                    ...
                    <tr>
                        ...
                        <td>{{ $r['class']->name() }}</td>
                        <td>@if($leader)<input type="text" name="numbers[{{ $s->id }}]" value="{{ $s->maneb_exam_number }}" style="width:150px">@else{{ $s->maneb_exam_number ?: 'Not issued' }}@endif</td>
    ...
    <div class="stack">
        <div class="panel">
            ...
            <form method="POST" action="{{ route('school.maneb.export') }}">@csrf
                <div class="panel-body stack-sm">
                    ...
                    @unless ($zipEncryption)<div class="alert warn" style="margin:0">Enable <strong>extension=zip</strong> in php.ini for AES encrypted exports.</div>@endunless
                </div>
        ...
        <div class="panel">
            ...
            <div class="panel-body small">
                ...
                <p style="margin-bottom:0"><strong>MSCE</strong> Six credits including English. Grades 1 and 2 are distinctions, 3 to 6 credits, 7 and 8 passes and 9 a fail.</p>
            </div>
        </div>
    </div>
</div>
